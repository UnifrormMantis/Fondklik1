    ...
    async def button_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            if data == "deposit":
                ...
                await self.show_admin_referral_payment_history(update, context)
    ...
    async def show_admin_deposit_payment_history(self, update: Update, context: ContextTypes.DEFAULT_TYPE, page: int = 0):
        ...
        try:
            with sqlite3.connect(DATABASE_PATH) as conn:
                ...
                # Получаем записи для текущей страницы (3 записи на страницу)
                offset = page * 3
                cursor.execute('''
            ...
            total_pages = (max_records + 2) // 3  # Пересчитываем страницы для 3 записей на страницу
...
"""
            ...
            for deposit in deposits:
                ...
                # Формат: сумма, имя, дата на одной строке, кошелек на отдельной строке
                message_text += f"\n✅ {amount:.0f}$ | {first_name} | {date_str}"
                message_text += f"\n💳 {wallet_full}"
    ...
    async def show_admin_referral_payment_history(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
...
📊 Всего: {total_referrals} | Сумма: {total_amount:.0f}$ | Последние 20:
"""
            
            for referral in referrals:
                amount, created_at, referrer_name, referrer_username, referred_name, referred_username, level, referrer_wallet = referral
                message_text += f"\n✅ {amount:.0f}$ | {referrer_name} | {referrer_wallet or 'N/A'} | L{level} | {created_at[:10]}"
