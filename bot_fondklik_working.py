...
• Система автоматически определит и зачислит полученную сумму"""
    ...
    async def show_referral_info(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        stats = self.get_user_referral_stats(user.id)
...
📊 Ваша статистика:
• 1-й уровень: {stats['level_1_count']} человек
• 2-й уровень: {stats['level_2_count']} человек
• 3-й уровень: {stats['level_3_count']} человек
• Заработано: {stats['total_earned']:.2f} USDT
• К выплате: {stats['pending_earnings']:.2f} USDT
        ...
        try:
            with sqlite3.connect(DATABASE_PATH) as conn:
                ...
                for level, count in cursor.fetchall():
                    if level == 1:
                        stats['level_1_count'] = count
                    elif level == 2:
                        stats['level_2_count'] = count
                    elif level == 3:
                        stats['level_3_count'] = count
                ...
                ''', (user_id,))
                result = cursor.fetchone()
                stats['pending_earnings'] = result[0] if result else 0.0
